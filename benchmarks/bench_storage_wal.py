...
def measure_wal(
    ...
    for t in TABLES:
        ...
        if t == "docs_tnt":
            ...
            # tokenizer, and the extension only ever sees token IDs. Encoding uses the same
            # reference client the extension is byte-checked against (test_client.py).
            id_lists = C.tokenize_all(texts)
            payloads = [K.encode(ids, C.DEFAULT_CODEC, vocab_id, table) for ids in id_lists]
        else:
            payloads = texts
...
def run_one(conn, texts: list[str], domain: str, embedding_bytes: int) -> dict:
    C.apply_schema(conn)
    vocab_id, table = C.ensure_freq_vocab(conn, domain)
    wal = measure_wal(conn, texts, vocab_id, table, embedding_bytes)
